...
5. `Tax_imp` / `Tax_dom` (`∑` each) / `Tax ∑∑` — taxes on products purchased as intermediate inputs, row = product purchased, column = buying sector, same convention as every other block (e.g. `Tax_imp` row `Equipment` at column `Chemical` = tax Chemical paid buying imported Equipment). `∑` sums each column, giving buyer j's own total tax paid across all products, at column j — this is the buyer-side figure `check_unbalance`/`Ressource_tot` use — and `Tax ∑∑ = Tax_imp ∑ + Tax_dom ∑`. This convention allows computing the output valued at basic prices by summing down the columns, consistent with the national accounting convention:
